...
package kokabiel.samurai.gui.screens;
...
import kokabiel.samurai.gui.components.widgets.SamuraiButtonWidget;
import kokabiel.samurai.gui.components.widgets.SamuraiImageButtonWidget;
import kokabiel.samurai.gui.screens.addons.AddonScreen;
...
public class MainMenuScreen extends Screen
{
    ...
    {
        super(Text.of("Samurai Client Main Menu"));
...
//    }
    ...
    public void init()
    {
        ...
        SamuraiButtonWidget singleplayerButton = new SamuraiButtonWidget(startX, startY, BUTTON_WIDTH, BUTTON_HEIGHT,
        ...
        SamuraiButtonWidget multiplayerButton = new SamuraiButtonWidget(startX, startY + BUTTON_HEIGHT + SPACING,
        ...
        SamuraiButtonWidget settingsButton = new SamuraiButtonWidget(startX, startY + ((BUTTON_HEIGHT + SPACING) * 2),
        ...
        SamuraiButtonWidget addonsButton = new SamuraiButtonWidget(startX, startY + ((BUTTON_HEIGHT + SPACING) * 3),
        ...
        SamuraiButtonWidget quitButton = new SamuraiButtonWidget(startX, startY + ((BUTTON_HEIGHT + SPACING) * 4),
                BUTTON_WIDTH, BUTTON_HEIGHT, Text.of("Quit"));
        quitButton.setPressAction(b -> client.stop());
        this.addDrawableChild(quitButton);

        SamuraiImageButtonWidget creditsButton = new SamuraiImageButtonWidget(this.width - 20 - 10, this.height - 20 - 10, 20,
                20, TextureBank.samurai);
        creditsButton.setPressAction(b -> MC.setScreen(new SamuraiCreditsScreen()));
        this.addDrawableChild(creditsButton);

        SamuraiImageButtonWidget discordButton = new SamuraiImageButtonWidget(this.width - 60, this.height - 30, 20, 20,
                TextureBank.discord);
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float delta)
    {
        ...
        drawContext.drawTextWithShadow(this.textRenderer, "Samurai " + SamuraiClient.SamuraiVersion, 2, this.height - 10,
                0xFF00FF);

        // Draw out of date if out of date.
        // TODO: Add option to hide if on previous versions.
        if (fetchedVersion != null && !fetchedVersion.equals(SamuraiClient.SamuraiVersion))
        {
            drawContext.drawTextWithShadow(this.textRenderer, "New version available: " + fetchedVersion, 2,
                    this.height - 20, 0xFF00FF);
        }

        if (SamuraiClient.addons.isEmpty())
        {
        ...
        {
            int yOffset = 10;
            for (IAddon addon : SamuraiClient.addons)
            {
        ...
        int newsTextWidth = this.textRenderer.getWidth("Samurai Started Development!") + 10;
        ...
        drawContext.drawTextWithShadow(this.textRenderer, "Samurai Started Development!", width - newsTextWidth - 5, 40, Colors.WHITE);
