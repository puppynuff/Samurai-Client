...
import kokabiel.samurai.gui.components.ColorPickerComponent;
import kokabiel.samurai.gui.components.KeybindComponent;
...
public class HudOptionsWindow extends Window {
	public HudOptionsWindow() {
		...
		List<String> fontNames = Samurai.getInstance().fontManager.fontRenderers.keySet().stream().toList();
		...
		KeybindComponent clickGuiKeybindComponent = new KeybindComponent(Samurai.getInstance().guiManager.clickGuiButton);
		...
		ListComponent listComponent = new ListComponent(fontNames, Samurai.getInstance().fontManager.fontSetting);
