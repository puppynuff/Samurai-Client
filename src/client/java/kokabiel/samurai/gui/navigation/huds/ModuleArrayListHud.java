...
public class ModuleArrayListHud extends HudWindow {
	...
	public ModuleArrayListHud(int x, int y) {
		...
		float newWidth = 0;
		for (Module mod : SamuraiClientlient.moduleManager.modules) {
	...
	public void draw(DrawContext drawContext, float partialTicks) {
		if (isVisible()) {
			...
			if (pos.isDrawable()) {
				...
				Stream<Module> moduleStream = SamuraiClientlient.moduleManager.modules.stream().filter(s -> s.state.getValue())
