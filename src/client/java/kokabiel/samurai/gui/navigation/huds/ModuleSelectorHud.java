...
public class ModuleSelectorHud extends HudWindow {
	...
	public ModuleSelectorHud() {
		...
		this.keybindUp = new KeyBinding("key.tabup", GLFW.GLFW_KEY_UP, "key.categories.samurai");
		this.keybindDown = new KeyBinding("key.tabdown", GLFW.GLFW_KEY_DOWN, "key.categories.samurai");
		this.keybindLeft = new KeyBinding("key.tableft", GLFW.GLFW_KEY_LEFT, "key.categories.samurai");
		this.keybindRight = new KeyBinding("key.tabright", GLFW.GLFW_KEY_RIGHT, "key.categories.samurai");
	}
	...
	public void update() {
		if (this.keybindUp.isPressed()) {
			...
			if (!isCategoryMenuOpen) {
				...
				if (modules.isEmpty()) {
					for (Module module : SamuraiClientlient.moduleManager.modules) {
